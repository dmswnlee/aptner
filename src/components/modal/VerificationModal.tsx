...
const VerificationModal = ({ onClose }: modalProps) => {
	const dispatch = useDispatch();
	const { isVerified, isExpired } = useSelector((state: RootState) => state.verification);
	...
	// 폼 제출할 때 (여기는 민재님 api에 맞춰서 하면 될거같아요!)
					...
					<div className="w-full flex flex-col gap-20">
						...
						<div className="flex flex-col items-center gap-[50px]">
							<form className="flex flex-col gap-[16px]">
								...
								<div className="flex items-center gap-[10px]">
									...
									<div className="flex flex-col gap-2">
										...
										<div>
											{errors.idNumber && (
												<p className="text-sm text-red">{getErrorMessage(errors.idNumber)}</p>
											)}
											...
											<div className="flex flex-col gap-2">
												<div className="flex">
													<button
														type="button"
														className={`${
															field.value === "male" ? buttonClickedStyle : buttonStyle
														} rounded-l-lg`}
														onClick={() => {
															field.onChange("male");
															setGender("male");
												...
												{errors.gender && (
													<p className="text-sm text-red">{getErrorMessage(errors.gender)}</p>
												)}
											</div>
										)}
									/>
								...
								<div className="flex gap-[10px]">
									...
									<div className="flex flex-col gap-2">
										...
										{isVerified && <p className="text-sm text-blue-500">인증이 완료되었습니다.</p>}
									</div>
									<SmallBorderButton
										...
										<div className="flex flex-col gap-2">
											<div className="relative">
												<input
													...
													className={`${inputStyle} ${
														errors.verificationCode || verificationError || isExpired
															? inputErrorStyle
															: ""
							...
							<div className="flex justify-center">
								<ColorButton text="변경 하기" size="lg" disabled={!isValid || !isVerified} />
							</div>
						</div>
					</div>
				</div>
